    ...
    public class RegistryConfigurationSource : AbstractConfigurationSource, IDisposable
    {
        ...
        private IEnumerable<IConfigurationSection> GetSections( RegistryKey root )
        {
            ...
            foreach ( string subKeyName in subkeys )
            {
                ...
                if ( !Recursive || string.Equals( root.Name, subKeyName ) )
                {
                    continue;
                }

                var subKey = OpenKey( subKeyName, RegistryKeyPermissionCheck.ReadSubTree );
                var sections = GetSections( subKey );
        ...
        private IEnumerable<string> GetSubKeys( RegistryKey registryKey )
        {
            foreach ( string subKeyName in registryKey.GetSubKeyNames() )
            {
                if ( Recursive )
                {
                    ...
                    var subTree = registryKey.OpenSubKey( subKeyName, permissionCheck );
        ...
        {
            var clone = OpenKey( source.Name, permissionCheck );
            return clone;
        }
