...
// 
// Copyright (c) 2009-2011, Ian Davis <ian@Lucid.com>
// 
...
namespace Lucid.Configuration
{
    ...
    public abstract class AbstractConfigurationSource : IConfigurationSource
    {
        ...
        {
            Debug.WriteLine( string.Format( Text.Culture, "Key [{0}]'s value changed in [{1}] section.", key, section ) );
            if ( AutoSave )
            {
                Save();
            }
        ...
        private string GetResultFromExternalSection( string sectionName, string keyName )
        {
            ...
            if ( newConfig == null )
            {
                throw new ArgumentException( string.Format( Text.Culture, Text.ExpandSectionMissing0, sectionName ) );
            }
            var result = newConfig.Get<string>( keyName );
            if ( result == null )
            {
                throw new ArgumentException( string.Format( Text.Culture, Text.ExpandKeyMissing0, keyName ) );
            }
            return result;
        }
        ...
        private static string GetResultFromCurrentSection( string keyName, IConfigurationSection section )
        {
            ...
            if ( result == null )
            {
                throw new ArgumentException(
                    string.Format( Text.Culture, Text.Key0NotFoundInSection1DuringExpansion, keyName, section.Name ),
                    keyName );
            }
            return result;
        }
